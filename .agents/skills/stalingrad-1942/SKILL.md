...
| `rl/train_ppo.py` | CleanRL single-file Maskable PPO training loop with GAE, self-play, and status JSON reporting |
| `rl/agent.py` | `RLAgent` inference wrapper matching `ai.play_turn`, plus head-to-head evaluation utilities |
| `rl/train_manager.py` | Non-blocking background subprocess training manager, status tracker, and model loader |
| `templates/index.html` | Board page shell (side panels, overlay, buttons, AI controls, RL Training Hub) |
| `static/css/style.css` | Layout + winter Stalingrad theme, training dashboard styles |
...
| `test_app.py` | Flask API integration + regression cycle + AI & training endpoints (23) |
